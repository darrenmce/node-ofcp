...
Game.prototype = {
  ...
  /* Will always play against the player who goes after AI player */
  aiTurn: function () {
    var self = this;
    if (self.gameStatus.started) {
      var currPlayer = self.getPlayer(self.gameStatus.turn)
        , oppPlayer = self.getPlayer(self.gameStatus.turnOrder[
          (self.gameStatus.turnOrder.indexOf(self.gameStatus.turn) + 1) % self.gameStatus.turnOrder.length
          ]);
      //format request
      var p1 = currPlayer.getBoard();
      var p2 = oppPlayer.getBoard();
      var open = currPlayer.unplayed;
      var jsonState = {"p1": p1, "p2": p2, "open": open};
      ...
      $.ajax({
        ...
        data: jsonState
          ...
          //play the cards
          if (data.data && data.data.bottom && data.data.mid && data.data.top) {
            data.data.bottom.forEach(function (card) {
              currPlayer.playCard(0, card.toLowerCase());
            });
            data.data.mid.forEach(function (card) {
              currPlayer.playCard(1, card.toLowerCase());
            });
            data.data.top.forEach(function (card) {
              currPlayer.playCard(2, card.toLowerCase());
            });
            self.endTurn(true);
            GameFunc.gameSync();
          }
      ...
      console.warn('Game not started, cannot play AI turn');
    }
  }
};